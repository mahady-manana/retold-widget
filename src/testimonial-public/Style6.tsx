...
function StarRating({ rating = 5 }: { rating?: number }) { return (<div className="flex items-center gap-0.5">{Array.from({ length: 5 }).map((_, i) => (<Star key={i} className={`w-3.5 h-3.5 ${i < rating ? "text-orange-400 fill-orange-400" : "text-gray-300"}`} />))}</div>); }
...
export default function WarmTestimonial({ testimonial }: { testimonial: PublicTestimonial }) {
  ...
  return (<>
    <div className="rounded-2xl bg-gradient-to-br from-orange-50 via-amber-50 to-yellow-50 p-6 border border-amber-200 shadow-md">
      <div className="flex justify-end mb-4"><div className="px-3 py-1.5 bg-white rounded-full shadow-sm border border-amber-200"><StarRating rating={testimonial.rating || 5} /></div></div>
      {hasTextContent && (<p className="text-sm text-gray-700 mb-5 line-clamp-4 italic border-l-4 border-amber-400 pl-4 bg-white/70 py-2 pr-3 rounded-r-lg">{testimonial.content}</p>)}
      {(hasVideo || hasAudio) && (<div className="flex gap-3 mb-5">
        {hasVideo && (<button onClick={() => setIsVideoModalOpen(true)} className="group w-16 h-16 flex-shrink-0 rounded-2xl overflow-hidden bg-gradient-to-br from-orange-500 to-amber-500 hover:shadow-lg hover:scale-105 transition-all relative" type="button"><div className="w-9 h-9 rounded-full bg-white/90 flex items-center justify-center group-hover:scale-110 transition-transform absolute inset-0 m-auto"><Play className="w-4 h-4 text-orange-600 ml-0.5" fill="currentColor" /></div><div className="absolute bottom-1 right-1 bg-black/80 text-white px-1.5 py-0.5 rounded-lg text-xs flex items-center gap-0.5"><Video className="w-2.5 h-2.5" />{formatDuration(testimonial.metadata?.videoDurationSeconds)}</div></button>)}
        {hasAudio && (<button onClick={() => setIsAudioModalOpen(true)} className="group w-16 h-16 flex-shrink-0 rounded-2xl overflow-hidden bg-gradient-to-br from-amber-500 to-yellow-500 hover:shadow-lg hover:scale-105 transition-all relative" type="button"><div className="w-9 h-9 rounded-full bg-white/90 flex items-center justify-center group-hover:scale-110 transition-transform absolute inset-0 m-auto"><Mic className="w-4 h-4 text-amber-600" /></div><div className="absolute bottom-1 right-1 bg-black/80 text-white px-1.5 py-0.5 rounded-lg text-xs flex items-center gap-0.5"><Mic className="w-2.5 h-2.5" />{formatDuration(testimonial.metadata?.audioDurationSeconds)}</div></button>)}
      </div>)}
      <div className="flex items-center gap-3 pt-4 border-t border-amber-200">
        <Avatar className="w-11 h-11 ring-2 ring-amber-300">{testimonial.authorProfilePhoto ? (<AvatarImage src={testimonial.authorProfilePhoto} alt={testimonial.authorName} />) : (<AvatarFallback className="bg-gradient-to-br from-orange-400 to-amber-400 text-white font-bold text-sm">{testimonial.authorName.split(" ").map((n) => n[0]).join("")}</AvatarFallback>)}</Avatar>
