...
function StarRating({ rating = 5 }: { rating?: number }) { return (<div className="flex items-center gap-0.5">{Array.from({ length: 5 }).map((_, i) => (<Star key={i} className={`w-3.5 h-3.5 ${i < rating ? "text-lime-400 fill-lime-400" : "text-gray-300"}`} />))}</div>); }
...
export default function FreshTestimonial({ testimonial }: { testimonial: PublicTestimonial }) {
  ...
  return (<>
    <div className="rounded-2xl bg-gradient-to-br from-lime-50 via-green-50 to-emerald-50 p-6 border-2 border-lime-200 shadow-lg">
      <div className="flex justify-end mb-4"><div className="px-3 py-1.5 bg-white rounded-full shadow-md border border-lime-300"><StarRating rating={testimonial.rating || 5} /></div></div>
      {hasTextContent && (<p className="text-sm text-gray-700 mb-5 line-clamp-4 italic border-l-4 border-lime-500 pl-4 bg-white/80 py-2 pr-3 rounded-r-lg">{testimonial.content}</p>)}
      {(hasVideo || hasAudio) && (<div className="flex gap-3 mb-5">
        {hasVideo && (<button onClick={() => setIsVideoModalOpen(true)} className="group w-16 h-16 flex-shrink-0 rounded-2xl overflow-hidden bg-gradient-to-br from-lime-500 to-green-500 hover:shadow-xl hover:scale-105 transition-all relative" type="button"><div className="w-9 h-9 rounded-full bg-white/90 flex items-center justify-center group-hover:scale-110 transition-transform absolute inset-0 m-auto shadow-lg"><Play className="w-4 h-4 text-lime-700 ml-0.5" fill="currentColor" /></div><div className="absolute bottom-1 right-1 bg-black/80 text-white px-1.5 py-0.5 rounded-lg text-xs flex items-center gap-0.5"><Video className="w-2.5 h-2.5" />{formatDuration(testimonial.metadata?.videoDurationSeconds)}</div></button>)}
        {hasAudio && (<button onClick={() => setIsAudioModalOpen(true)} className="group w-16 h-16 flex-shrink-0 rounded-2xl overflow-hidden bg-gradient-to-br from-green-500 to-emerald-500 hover:shadow-xl hover:scale-105 transition-all relative" type="button"><div className="w-9 h-9 rounded-full bg-white/90 flex items-center justify-center group-hover:scale-110 transition-transform absolute inset-0 m-auto shadow-lg"><Mic className="w-4 h-4 text-green-700" /></div><div className="absolute bottom-1 right-1 bg-black/80 text-white px-1.5 py-0.5 rounded-lg text-xs flex items-center gap-0.5"><Mic className="w-2.5 h-2.5" />{formatDuration(testimonial.metadata?.audioDurationSeconds)}</div></button>)}
      </div>)}
      <div className="flex items-center gap-3 pt-4 border-t-2 border-lime-200">
        <Avatar className="w-11 h-11 ring-2 ring-lime-400 shadow-md">{testimonial.authorProfilePhoto ? (<AvatarImage src={testimonial.authorProfilePhoto} alt={testimonial.authorName} />) : (<AvatarFallback className="bg-gradient-to-br from-lime-400 to-green-400 text-white font-bold text-sm shadow-md">{testimonial.authorName.split(" ").map((n) => n[0]).join("")}</AvatarFallback>)}</Avatar>
